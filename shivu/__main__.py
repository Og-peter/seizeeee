...
async def send_image(update: Update, context: CallbackContext) -> None:
    ...
    message = await context.bot.send_photo(
        ...
        caption=f"""<b>{rarity_emoji} ᴋᴀᴡᴀɪ! ᴀ {selected_character['rarity'][2:]} ᴄʜᴀʀᴀᴄᴛᴇʀ ʜᴀs ᴀᴘᴘᴇᴀʀᴇᴅ!</b>\n
...
async def placeholder_callback(update: Update, context: CallbackContext):
 ...
 async def guess(update: Update, context: CallbackContext) -> None:
    ...
    # Validate the guess for special characters
    ...
    if sorted(name_parts) == sorted(guess.split()) or guess in name_parts:
