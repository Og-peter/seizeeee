...
async def hmode_callback(client, callback_query: CallbackQuery):
    ...
    if data == "sort_rarity":
        ...
        await send_sorted_characters(callback_query, sort_by="name")
    elif data == "sort_anime":
        await send_sorted_characters(callback_query, sort_by="anime")
...
async def send_sorted_characters(callback_query: CallbackQuery, sort_by, page=1):
    ...
    reply_markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("⬅️ Prev", callback_data=f"sort_{sort_by}_prev_{page}") if page > 1 else None,
         InlineKeyboardButton("➡️ Next", callback_data=f"sort_{sort_by}_next_{page}") if page < total_pages else None]
    ])
...
async def paginate_sorted_characters(client, callback_query: CallbackQuery):
    ...
    await send_sorted_characters(callback_query, sort_by=sort_by, page=page)
...
async def send_random_character(callback_query: CallbackQuery):
    ...
    caption = f"🎲 Randomly Selected:\n\n🧃 Name: {random_character['name']}\n⚜️ Anime: {random_character['anime']}\n🥂 Rarity: {random_character['rarity']}"
    
    await callback_query.message.reply_photo(
        photo=random_character.get("img_url"),
        caption=caption
    )
    await callback_query.answer()
